...
local function Drawable(parameters, fromObject)
    ...
    local _drawBuffer = parameters.drawBuffer or -1

    function instance:getDrawBuffer() return _drawBuffer end
    function instance:setDrawBuffer(value) _drawBuffer = value end
    ...
    function instance:drawRectangle(x, y, w, h, filled)
        local x = x + _point:getX()
        local y = y + _point:getY()
    ...
    function instance:drawLine(x, y, x2, y2, antiAliased)
        local x = x + _point:getX()
        local y = y + _point:getY()
        local x2 = x2 + _point:getX()
        local y2 = y2 + _point:getY()
        gfx.dest = _drawBuffer
    ...
    function instance:drawString(str, x, y, flags, right, bottom)
        local x = x + _point:getX()
        local y = y + _point:getY()
        local right = right + _point:getX()
        local bottom = bottom + _point:getY()
        gfx.dest = _drawBuffer
        gfx.x = x
        gfx.y = y
        if flags then
