...
function Track:getNumber()         return reaper.GetMediaTrackInfo_Value(self.pointer, "IP_TRACKNUMBER") end
function Track:getItemCount()      return reaper.GetTrackNumMediaItems(self.pointer) end
function Track:getItem(itemNumber) return self:getProject():wrapItem(reaper.GetTrackMediaItem(self.pointer, itemNumber - 1)) end
...
function Track:getSelectedItemCount()
    ...
    for index, itemNumber in ipairs(selectedItemNumbers) do
        if reaper.IsMediaItemSelected(reaper.GetTrackMediaItem(self.pointer, itemNumber - 1)) then
